...
def readTrendCentralData(filepath):
    ...
    try:
        rawdata = pd.read_table(filepath, sep='\t', index_col = False,
                                names=['SeqNum','StartLineIndex','EndLineIndex','SmallTcCount','Level','TrendType','High','Low','Highest', 'Lowest'],
                                dtype={'SeqNum': np.int, 'StartLineIndex': np.int, 'EndLineIndex': np.int})
    except IOError:
        raise Exception('IoError when reading TrendCentral data file:' + filepath)

    # Wash data
    returndata = pd.concat([rawdata, returndata])
    returndata = returndata.reindex(columns=['SeqNum','StartLineIndex','EndLineIndex','Level','TrendType','High','Low','Highest', 'Lowest'])
    return returndata
...
class ZenMsData:
    ...
    def LoadSecuritiesKlineData(self, code, period, start_ts):
        ...
        try:
            ...
            sec_sql = "SELECT ts, open, high, low, close, volume, money FROM " + db_name + "." + table_name \
                      + " WHERE gp_id='" + str(gp_id) + "' AND ts>='" + str(start_ts) + "' ORDER BY ts ASC"
            df = pd.read_sql(sql=sec_sql, con=self.engine)
            df['OriginalIndex'] = df.index
            df.rename(columns={'ts':'Ts','open':'Open', 'high':'High', 'low':'Low', 'close':'Close', 'volume':'Volume', 'money':'Amount'}, inplace=True)
    ...
    #获取gp的kline count
    def GetSecuritiesKlineCount(self, code, period):
        g_logger.info('GetSecuritiesKlineCount begin! code=%s, period=%s', code, period)
        db_name = ''
        if self.securities_type=='stock':
            db_name = 'gp'
        elif self.securities_type=='index':
        ...
        try:
            table_name = period + "_prices_" + codes[0][-2:]
            sec_sql = "SELECT count(1) FROM " + db_name + "." + table_name + " WHERE gp_id='" + str(gp_id) + "'"
            mycursor.execute(sec_sql)
            # 获取所有记录列表
            results = mycursor.fetchall()
            count =0
